...
The First Group — YeGaoh (Tongues 1–24):
  Factorization signature: 2^3*3 = 24
...
F_24:  Final = factorize(24)   # 2^3*3   — YeGaoh Group signature / T1–T3
F_26:  Final = factorize(26)   # 2*13    — T4–T5
F_28:  Final = factorize(28)   # 2^2*7   — T6–T7
F_30:  Final = factorize(30)   # 2*3*5   — T8–T11
F_32:  Final = factorize(32)   # 2^5     — T12–T13
F_34:  Final = factorize(34)   # 2*17    — T14–T17 (Immune deviation: +1 tongue)
F_36:  Final = factorize(36)   # 2^2*3^2 — T18–T19
F_38:  Final = factorize(38)   # 2*19    — T20–T23
F_40:  Final = factorize(40)   # 2^3*5   — T24–T29
F_42:  Final = factorize(42)   # 2*3*7   — T30–T31
F_44:  Final = factorize(44)   # 2^2*11  — T32–T36 (Moon opens; range extends to T36)
...
TONGUE_REGISTRY: Final[Tuple[TongueRecord, ...]] = (
    ...
    # Signature: 2^3*3 = 24  |  3 clusters of 8
    # -----------------------------------------------------------------------
    ...
    TongueRecord(32, "Moon",          44, F_44, 1094, 1137, group=2, cluster=1, cluster_position=8, opening_prime=31,
                 notes="Elemental meta-mapping tongue: 11 roots x 4 elemental bands (Earth/Water/Air/Fire). "
...
GROUP_REGISTRY: Final[Tuple[GroupRecord, ...]] = (
    GroupRecord(
        ...
        tongue_range=(1, 24),
        tongue_count=24,
        factorization=F_24,   # 2^3*3 — same as opening tongues' entry count
    ...
    GroupRecord(
        ...
        factorization=F_40,   # opening tongue (Fold, T25) has 40 entries — 2^3*5
        cluster_boundaries=(),
    ),
)
