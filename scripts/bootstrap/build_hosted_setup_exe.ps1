param(
    [string]$OutExePath = "c:\DjinnOS\releases\KoLabyrnth-Hosted-Setup.exe",
...
$wizardCmd = Join-Path $bootstrapRoot "Setup-Hosted-Wizard.cmd"
$wizardPs1 = Join-Path $bootstrapRoot "Setup-Hosted-Wizard.ps1"
$installerPs1 = Join-Path $bootstrapRoot "Install-Hosted-Atelier.ps1"
...
$configTemplate = Join-Path $bootstrapRoot "config.hosted.template.json"
$readme = Join-Path $bootstrapRoot "README-hosted-quickstart.txt"
...
$tempRoot = Join-Path $env:TEMP ("atelier-hosted-setup-exe-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    Copy-Item -Force $wizardCmd (Join-Path $stageDir "Setup-Hosted-Wizard.cmd")
    Copy-Item -Force $wizardPs1 (Join-Path $stageDir "Setup-Hosted-Wizard.ps1")
    Copy-Item -Force $installerPs1 (Join-Path $stageDir "Install-Hosted-Atelier.ps1")
    Copy-Item -Force $launcherPs1 (Join-Path $stageDir "Launch-Atelier.ps1")
    Copy-Item -Force $iconPath (Join-Path $stageDir "icon.ico")
    Copy-Item -Force $configTemplate (Join-Path $stageDir "config.hosted.template.json")
    Copy-Item -Force $readme (Join-Path $stageDir "README-hosted-quickstart.txt")
...
setlocal
cd /d "%~dp0"
powershell -NoProfile -ExecutionPolicy Bypass -File ".\Setup-Hosted-Wizard.ps1"
endlocal
...
InstallPrompt=
DisplayLicense=
FinishMessage=Ko's Labyrnth hosted setup is ready.
TargetName=$OutExePath
FriendlyName=Ko's Labyrnth Hosted Setup
...
"@ | Set-Content -Encoding ASCII -LiteralPath $sedPath
    ...
    if ($SignMode -eq "selfsigned") {
        $signCert = New-SelfSignedCertificate `
            ...
            -Subject "CN=Ko's Labyrnth Hosted Installer Signer" `
